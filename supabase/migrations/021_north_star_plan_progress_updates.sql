-- Let executive users update BPR plan/actual/forecast through the progress RPC.
...
begin
  ...
  set plan_value = nullif(btrim(coalesce(p_plan_value, '')), ''),
      actual_mtd = nullif(btrim(coalesce(p_actual_mtd, '')), ''),
      forecast = nullif(btrim(coalesce(p_forecast, '')), ''),
      constraint_now = nullif(btrim(coalesce(p_constraint_now, '')), ''),
  ...
  from (
    values
      ('plan_value', previous.plan_value, updated.plan_value),
      ('actual_mtd', previous.actual_mtd, updated.actual_mtd),
      ('forecast', previous.forecast, updated.forecast),
...
revoke all on function public.update_north_star_progress(uuid, text, text, text, text, text, text, text) from public;
revoke all on function public.update_north_star_progress(uuid, text, text, text, text, text, text, text) from anon;
grant execute on function public.update_north_star_progress(uuid, text, text, text, text, text, text, text) to authenticated;
