...
describe('Sales by Channel data contract', () => {
  ...
  it('builds the executive page around mapped sales, month navigation, goals, and Add mapping', () => {
    ...
    expect(pageSource.indexOf('const sortedRows = useMemo')).toBeGreaterThan(-1)
    expect(pageSource.indexOf('const sortedRows = useMemo')).toBeLessThan(pageSource.indexOf('if (isLoading) return <PageLoader />'))
    ...
    expect(pageSource).toContain('Goal vs MTD')
    expect(pageSource).toContain('Goal vs projected')
    expect(pageSource).toContain('Game plan')
    expect(pageSource).toContain('useUpdateSalesChannelGamePlan')
    expect(pageSource).toContain('canEditGamePlan={selectedMonth === currentMonth')
  ...
  it('adds a month-scoped hook that fetches sales rows, mappings, and goals', () => {
    ...
    expect(hookSource).toContain("from('sales_channel_game_plans')")
  ...
  it('adds active-user monthly game plan mutation hooks', () => {
    expect(existsSync(hookPath)).toBe(true)
    const hookSource = readFileSync(hookPath, 'utf8')

    expect(hookSource).toContain('useUpdateSalesChannelGamePlan')
    expect(hookSource).toContain('SalesChannelGamePlan')
    expect(hookSource).toContain("onConflict: 'period_month,qb_channel'")
    expect(hookSource).toContain("invalidateQueries({ queryKey: ['sales_by_channel', payload.period_month] })")
  })
})
