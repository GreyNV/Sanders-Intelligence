...
describe('Stitch North Star page contract', () => {
  ...
  it('renders the PnL slide through a responsive NOI forecast panel', () => {
    ...
    expect(pageSource).toContain('OpEx run-rate')
    ...
    expect(pageSource).toContain('Var + OpEx')
    expect(pageSource).toContain('North Star projected sales')
    expect(pageSource).toContain('formatCompactCurrency(month.forecastedSales)')
    expect(pageSource).toContain('group-hover:block')
    expect(pageSource).toContain('tabIndex={0}')
    expect(pageSource).toContain('Fixed OpEx')
    expect(pageSource).toContain('forecastedSalesRateCosts')
  ...
  it('renders per-card Fields/HTML view modes with raw sandboxed iframe srcdoc', () => {
    ...
    const frameSource = readFileSync(resolve(__dirname, '../components/SlideFrame.tsx'), 'utf8')
    expect(frameSource).toContain('sandbox="allow-same-origin"')
    expect(frameSource).toContain('srcDoc={html}')
